...
| **PnL net** | `brut − frais − gas − rebalancing − capital − hedge − provision de risque op.` (formule **unique** à 7 termes, `compute_net_pnl`, MISSION RESET §7). Niveau **2** si positif à taille définie. **hedge / provision de risque op. : jamais un 0 silencieux** (None ⇒ abstention ; 0.0 explicite par le caller). | Jamais une somme d'opportunités au même instant (capital/liquidité partagés). |
...
| **identité (3 niveaux prouvés + plancher)** | `IDENTITY_PRELIMINARY` (affirmé, non prouvé — aucun gate) puis `CONTRACT_SAME` < `ECONOMIC_IDENTITY_CONFIRMED` < `REBALANCING_CONFIRMED` — **jamais équivalents** (`sim/economic_identity.py`). Les paliers économiques exigent un **`evidence_hash`** (reçu archivé+hashé), jamais un simple `evidence_url`. | Une même adresse ne prouve **pas** l'identité économique ni le rebalancing (leçon CTM). Un pointeur n'est pas un reçu (leçon VELVET). |
