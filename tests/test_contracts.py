...
On epingle les DEUX invariants codes :
  - la formule de PnL net est UNIQUE (compute_net_pnl) et reproduit docs/STATE.md §6 ;
  - ABSTENTION jamais fallback silencieux (§7) : un cout manquant ou une jambe sans sortie =>
    net NaN, confidence 0, missing_fields peuple — jamais un 0 invente.
"""
import math
...
# --- 1. Formule canonique unique ----------------------------------------------------------------

def test_compute_net_pnl_formule_canonique():
    # net = brut - frais - gas - rebalancing - capital  (STATE.md §6)
    assert compute_net_pnl(100.0, 10.0, 5.0, 3.0, 2.0) == 80.0
    # atomique mono-chaine : pas de rebalancing/capital
    assert compute_net_pnl(50.0, 12.0, 8.0) == 30.0
