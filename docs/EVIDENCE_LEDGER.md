...
| 7 | **Funding / cash-and-carry** — perp ETHUSDT, 1 an (2025-06-23→2026-06-23) | **NON_CONCLUANT** | **Réglé ex post CERTIFIÉ** : `runs/20260624T012503Z_phase1-binance-ethusdt-backfill/` · git `6f77a19` · **1095 règlements** (COLLECTE_COMPLETE), W1 sha256 `3c359271…` / W2 `b0cb5176…` (bruts hors Git). Faisabilité `…phase0b…` git `8928468` (SOURCE_CAPABLE). Observabilité 1.5 + plan 2A + **gate §2 bis** git `9ba5523`+ (niv. 3 **ex post**) | Une **série RÉGLÉE ex post certifiée** (1095, contiguë 8 h, 0 gap/0 doublon) — taux **encaissés au règlement** | **Aucun PnL, aucune stratégie** ; surtout la **reconstruction EX ANTE est NON PROUVÉE** : `GATE_2BIS_NON_FRANCHIE` (2026-06-24) — correspondance OHLC `premiumIndexKlines` ↔ TWAP interne (5 s/5760) **non démontrée, non réfutée** ; `MODELE_VALIDE` **inatteignable avec les sources actuelles** | **Plafond `FIDELITE_MESUREE`** ; **aucune collecte `premiumIndexKlines` pour une simple `FIDELITE_MESUREE`**. **Réouverture (l'une OU l'autre)** : ① **source officielle** prouvant la correspondance OHLC ↔ échantillonnage TWAP ; OU ② **accès aux obs historiques 5 s réellement utilisées par Binance**. Puis cycle complet spot/perp avec **quotes exécutables** (fenêtre forward distincte) → PnL 7-termes + capacité/stress |
...
| Funding | ✅ **réglé ex post certifié** (Phase 1, 1095) ; ❌ **ex ante non prouvé reconstructible** (gate §2 bis non franchie) | NON_CONCLUANT — `MODELE_VALIDE` inatteignable (sources actuelles) |
...
Track **funding** : mécanisme documenté ; **série réglée ex post certifiée** (Phase 1, 1095 règlements) ;
mais la **reconstruction historique ex ante est NON PROUVÉE** (`GATE_2BIS_NON_FRANCHIE`, 2026-06-24 —
correspondance OHLC ↔ TWAP interne **non démontrée, non réfutée**). **`MODELE_VALIDE` inatteignable avec les
sources actuelles** ; **aucune collecte `premiumIndexKlines` pour une simple `FIDELITE_MESUREE`**.
Réouverture : source officielle prouvant la correspondance OHLC ↔ échantillonnage TWAP, **ou** accès aux
observations historiques **5 s** réellement utilisées par Binance. Détail :
`docs/mechanisms/funding_gate2bis_recon.md`.
