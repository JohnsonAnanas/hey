...
| 7 | **Funding / cash-and-carry** — perp, 21 actifs, 1 an quotidien | **NON_CONCLUANT** | `data/logs/funding_regime.csv` — **non certifiée, non suivie Git, sans manifeste** ; producteur `funding_regime.py` (ccxt binance/okx) ; **brut par marché ni archivé ni présent dans le projet** (niv. 0→1) | Un **flux de funding** annualisé **rapporté** (série **non certifiée**) | **Ni rendement net ni stratégie** ; `best_carry = max(exchange)` = **enveloppe haute choisie a posteriori → diagnostic rétrospectif, jamais un PnL exécutable** ; aucun basis exécutable ; `breadth=0` / plafond ~10,9 % invérifiables sans brut | **Orientation documentaire** (jamais LEAD). Une **future série doit conserver les obs par marché/venue** et **interdire toute sélection quotidienne a posteriori de la meilleure venue** (venue/règle **fixée avant la fenêtre de test**, sur **données observables avant le règlement**). Puis ② certifiés+hashés → ③ **cycle complet** spot/perp (entrée synchronisée → funding réellement réglé → sortie synchronisée) → ④ PnL 7-termes + capacité + stress |
